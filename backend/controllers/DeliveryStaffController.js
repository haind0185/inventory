...
const DeliveryStaffController = {
    ...
    index: async (req, res) => {
        try {
            ...
            if (req.query.DeliveryStaffName) {
                where.DeliveryStaffName = { [Op.like]: `%${req.query.DeliveryStaffName}%` }
            }

            const status = helper.parseBoolean(req.query.DeliveryStaffActive)
            if (status !== undefined) {
                where.DeliveryStaffActive = status
            }

            /**
             * order
             */
            const order_list = ['DeliveryStaffName', 'DeliveryStaffActive',]
            let order = []
    ...
    store: async (req, res) => {
        ...
        try {
            ...
            const { DeliveryStaffName } = req.body;

            /**
             * validation
             */
            const schema = Joi.object({
                DeliveryStaffName: Joi.string().required().min(1).max(200),
            }).unknown();
             ...
             * check exists code
             */
            const existsDeliveryStaff = await DeliveryStaff.findOne({
                where: {
                    DeliveryStaffName: DeliveryStaffName
                }
            }, { transaction: transaction })
            if (existsDeliveryStaff) {
    ...
    update: async (req, res) => {
        ...
        try {
            console.log(req.body)
            const { id, DeliveryStaffName, DeliveryStaffActive } = req.body;
            ...
            const schema = Joi.object({
                DeliveryStaffName  : Joi.string().required().min(1).max(200),
                DeliveryStaffActive: Joi.boolean().required(),
            }).unknown();
             ...
             * check exists code
             */
            const existsStaff = await DeliveryStaff.findOne({
                where: {
                    id: id
                },
            }, { transaction: transaction })
             ...
             * call create action
             */
            existsStaff.DeliveryStaffName   = DeliveryStaffName
            existsStaff.DeliveryStaffActive = DeliveryStaffActive
    ...
    delete: async (req, res) => {
        ...
        try {
            ...
            if(orders.length > 0) {
                throw new Error("Nhân viên giao nhận này đã có đơn hàng, không thể xóa.");
            }
...
export default DeliveryStaffController;
