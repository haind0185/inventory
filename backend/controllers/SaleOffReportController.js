...
const SaleOffReportController = {
    ...
    exportCustomer: async (req, res) => {
        try {
             ...
             * Export excel handle
             */
            const data = helper.transformDataCustomer(items)
            let sheetData = [['Khách hàng', 'Tháng', 'Đơn', 'Sản phẩm', 'S.L.1', 'S.L.2', 'Đơn giá', 'S.L', 'Thành tiền', 'Ghi chú']]
            let merges = []
            let currentRow = 1
            for (const staff of data) {
                const startRowStaff = currentRow;
                ...
                staff.Months.forEach((month, mIdx) => {
                    ...
                    month.Orders.forEach((order, oIdx) => {
                        ...
                        order.Products.forEach((product, pIdx) => {
                            sheetData.push([
                                currentRow === startRowStaff ? `[${staff.CustomerCode}] ${staff.CustomerName}\n${helper.format_number(staff.CustomerQty)}` : null,
                                currentRow === startRowMonth ? `${month.Name}\n${helper.format_number(month.MonthQty)}` : null,
                                currentRow === startRowOrder ? `${order.OrderCode}\n${order.OrderDate}\n${helper.format_number(order.OrderQty)}` : null,
                                product.ProductNameLabel,
                                product.LargeUnitQty,
                                product.SmallUnitQty,
                                product.Price,
            ...
            const workbook = xlsx.utils.book_new();
            xlsx.utils.book_append_sheet(workbook, worksheet, 'Sheet1');
            let filename = moment().format('YYYYMMDD_HHmmss') + '_bao_cao_khach_hang.xlsx';
            const r = await Service.downloadStyle(workbook, filename);
            console.log(r)
            if (r) {
                return res.json(
    ...
    exportSaleStaff: async (req, res) => {
        try {
             ...
             * Export excel handle
             */
            const data = helper.transformDataSaleStaff(items)
            let sheetData = [['NVBH', 'Tháng', 'Đơn', 'Sản phẩm', 'S.L.1', 'S.L.2', 'Đơn giá', 'S.L', 'Thành tiền', 'Ghi chú']]
            let merges = []
            let currentRow = 1
            for (const staff of data) {
                const startRowStaff = currentRow;
                ...
                staff.Months.forEach((month, mIdx) => {
                    ...
                    month.Orders.forEach((order, oIdx) => {
                        ...
                        order.Products.forEach((product, pIdx) => {
                            sheetData.push([
                                currentRow === startRowStaff ? `${staff.SaleStaffName}\n${helper.format_number(staff.SaleStaffQty)}` : null,
                                currentRow === startRowMonth ? `${month.Name}\n${helper.format_number(month.MonthQty)}` : null,
                                currentRow === startRowOrder ? `${order.OrderCode}\n${order.OrderDate}\n${helper.format_number(order.OrderQty)}` : null,
                                product.ProductNameLabel,
                                product.LargeUnitQty,
            ...
            const workbook = xlsx.utils.book_new();
            xlsx.utils.book_append_sheet(workbook, worksheet, 'Sheet1');
            let filename = moment().format('YYYYMMDD_HHmmss') + '_bao_cao_NVBH.xlsx';
            const r = await Service.downloadStyle(workbook, filename);
            console.log(r)
            if (r) {
                return res.json(
