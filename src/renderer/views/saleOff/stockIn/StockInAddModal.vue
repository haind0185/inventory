        ...
        <form class="flex flex-col justify-between h-full gap-1 p-2" style="height: 40rem;" @submit.prevent="onSave()">
            <div class="flex gap-4">
                <fieldset class="w-[20%] form-input required">
                    ...
                    <input type="text" class="w-full text-center form-control" required v-model="payload.StockInCode">
                </fieldset>
                <fieldset class="w-[20%] form-input required">
...
const title = 'Nhập hàng lưu kho'
const props = defineProps(['show'])
const emit = defineEmits(['close', 'save'])

const payload = computed(() => stockInStore.payload)
...
const onSave = async () => {
    ...
    if(ok) {
        ...
        const res = await stockInStore.store(payload.value).then((res) => {
            if(res && res.code == 200) {
                reload.value = true
                return true
            }
