            ...
            <table class="view-scroll t-border">
                <thead>
                    <tr>
                        ...
                        <th class="w-[4rem]">Tổng SL</th>
                        <th class="w-[4rem]">Tổng tiền</th>
                ...
                <tbody>
                    <template v-for="item in items.items">
                        <tr style="background: #dfe6f5; cursor: pointer;">
                            <td class="w-[2.5rem] text-center show-list" @click="item.show = !item.show">{{ item.saleOffStockInItems.length }}</td>
                        ...
                        <tr v-for="(i, index) in item.saleOffStockInItems" v-show="item.show">
                            <td class="text-center">{{ index+1 }}</td>
                            <td class="text-left">{{ i.saleOffProduct.ProductNameLabel }}</td>
                            <td class="text-right">{{ format_number(i.LargeUnitQty) }}</td>
                            <td class="text-right">{{ format_number(i.SmallUnitQty) }}</td>
                             <td class="text-right">{{ format_number(i.Price) }}</td>
                            <td class="text-right">{{ format_number(i.Qty) }}</td>
                            <td class="text-right">{{ format_number(i.PriceQty) }}</td>
                            <td class="!text-xs text-left">{{ i.StockInItemNote }}</td>
                        </tr>
                        <tr v-show="item.show">
                            ...
                            <td></td>
                            <td class="!font-bold text-right">
                                {{ format_number(item.saleOffStockInItems.reduce((sum, item) => sum + item.Qty, 0)) }}
                            </td>
                            <td class="!font-bold text-right">
                                {{ format_number(item.saleOffStockInItems.reduce((sum, item) => sum + item.PriceQty, 0)) }}
                            </td>
                            <td></td>
                        </tr>
                    </template>
...
const onShowDetail = async (StockInCode) => {
    await saleOffProductStore.list()
    await stockInStore.show({StockInCode: StockInCode}).then((res) => {
        if(res && res.code == 200) {
...
const onDelete = async (StockInCode) => {
    const ok = await confirm.value.show({
        title: t("title.confirm"),
        message: `Cân nhắc kỹ trước khi khóa.<br>Xác nhận xóa đơn nhập kho có mã: ${StockInCode}`,
        cancelButton: t("button.back"),
    })
    if(ok) {
        await confirm.value.close()
        await stockInStore.destroy({StockInCode: StockInCode}).then(async (res) => {
        if(res && res.code == 200) {
            await index()
        }
    })
