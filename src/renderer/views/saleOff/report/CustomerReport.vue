<template>
    <div class="gap-1 wrapper-scroll">
                ...
                <div class="flex w-full gap-3">
                    <fieldset class="form-input w-[50%]">
                        ...
                        <select2 class="w-full form-control"
                            ...
                            v-model="customerSearch.CustomerCodes"
                    ...
                    <fieldset class="form-input w-[30%]">
                        ...
                        <div class="flex gap-3">
                            <date class="w-full from-control" v-model="customerSearch.OrderDateFrom" :max-date="customerSearch.OrderDateTo"></date>
                            ~
                            <date class="w-full from-control" v-model="customerSearch.OrderDateTo" :min-date="customerSearch.OrderDateFrom"></date>
                        </div>
                    </fieldset>
                </div>
            </div>
        ...
        <div class="flex mt-5">
            <div class="w-[40%] flex">
                <Pagination v-if="customers.total" v-model="customerSearch.page" class="mb-0" :page-count="customers.page_count ?? 0" :click-handler="pagination"></Pagination>
        ...
        <div class="parent-scroll">
            <table class="view-scroll t-border">
                <thead>
                    <tr>
                        <th class="w-[10rem]">Khách hàng</th>
...
const customerSearch = computed(() => SOReport.searchCustomer)
const customers = ref({})
...
const index = async () => {
    await SOReport.customer(customerSearch.value).then((res) => {
        if(res && res.code == 200) {
            setData(res.data)
        }
    })
...
const pagination = (page) => {
    customerSearch.value.page = page
    index()
}

const exportFile = async () => {
    await SOReport.exportCustomer(customerSearch.value).then((res) => {
        if(res && res.code == 200) {
            console.log(res)
        }
    })
