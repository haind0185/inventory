<template>
    <div class="gap-1 wrapper-scroll">
            ...
            <div class="flex flex-col flex-1">
                <div class="flex w-full gap-3">
                    <fieldset class="form-input w-[50%]">
                        <legend>Nhân viên giao nhận</legend>
                        <select2 class="w-full form-control"
                            :multiple="true"
                            :clearable="true"    
                            :options="master.deliveryStaffs"
                            v-model="deliveryStaffSearch.Ids"
                    ...
                    <fieldset class="form-input w-[30%]">
                        ...
                        <div class="flex gap-3">
                            <date class="w-full from-control" v-model="deliveryStaffSearch.OrderDateFrom" :max-date="deliveryStaffSearch.OrderDateTo"></date>
                            ~
                            <date class="w-full from-control" v-model="deliveryStaffSearch.OrderDateTo" :min-date="deliveryStaffSearch.OrderDateFrom"></date>
                        </div>
                    </fieldset>
                </div>
            </div>
        ...
        <div class="flex mt-5">
            <div class="w-[40%] flex">
                <Pagination v-if="customers.total" v-model="deliveryStaffSearch.page" class="mb-0" :page-count="customers.page_count ?? 0" :click-handler="pagination"></Pagination>
                                ...
                                <template v-for="(product, productCount) in order.Products">
                                    <tr>
                                        ...
                                        <td class="text-center row-left" :rowspan="countNestedItems(order, ['Products'])" v-if="productCount == 0">
                                            ...
                                            {{ order.OrderDate }}<br>
                                            <span class="font-bold">
                                                {{ format_number(order.orderQty) }}{{ order.Workload > 1 ? ' / '+ order.Workload : '' }}
                                            </span>
                                        </td>
...
const deliveryStaffSearch = computed(() => SOReport.searchDeliveryStaff)
const customers = ref({})
...
const clear = async () => {
    SOReport.resetSearchDeliveryStaff()
    await index()
}
const index = async () => {
    await SOReport.deliveryStaff(deliveryStaffSearch.value).then((res) => {
        if(res && res.code == 200) {
            setData(res.data)
        }
    })
