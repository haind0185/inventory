<template>
    <Modal :show="show" title="Chi tiết đơn hàng" maxWidth="max-w-7xl" @close="onClose()" class="">
...
const {show, order} = defineProps(['show', 'order'])
...
const setDetail = (order) => {
    ...
    orderStore.setPayload(dataPayload)
    let dataSaleRoutes = order.saleRoutes
    orderStore.setSaleRoutes(dataSaleRoutes)
}
...
const onSave = async () => {
    ...
    const ok = await confirm.value.show({
        title: t("title.confirm"),
        message: 'Xác nhận chỉnh sửa đơn có tổng giá trị: '+getTotalPriceQty(),
        cancelButton: t("button.back"),
    })
    if(ok) {
